...
public class OrderConfirmController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean check = dao.confirmOrder(dateOrder,slotID,categoryID);
            if (check) {
                dao.confirmed(orderDetailID,slotID,dateOrder);
                
                request.setAttribute("CN", "Succesfully confirm!");
                url = SUCCESS;
            } else {
                request.setAttribute("CN", "There no Field avaible with slotID "+slotID+" + at "+dateOrder+"!");
                url = ERROR;
            }
        } catch (Exception e) {
